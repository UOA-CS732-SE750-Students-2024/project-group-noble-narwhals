...
describe("GET api/user/userData/:id", () => {
  test("should return user info in details for a valid user ID", async () => {
    ...
    expect(res.body.participatingGroups).toContainEqual(
      expect.objectContaining({
        _id: "000000000000000000000001",
        groupName: "group1",
      })
    );
    expect(res.body.likedGroups).toContainEqual(
      expect.objectContaining({
        _id: "000000000000000000000001",
        groupName: "group1",
      })
    );
    expect(res.body.appliedGroups).toContainEqual(
      expect.objectContaining({
...
describe("POST /like & unlike/:groupId", () => {
  ...
  test("should able to unlike a group", async () => {
    ...
    expect(unlikeResponse.body).toHaveProperty(
      "message",
      "Group unliked successfully"
    );
