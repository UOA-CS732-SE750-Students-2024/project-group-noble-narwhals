...
beforeAll(async () => {
    ...
    userRouter.get('/:id/likes/:groupId', async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ message: 'Error processing request', error: err.toString() });
        }
    });
    ...
    userRouter.post('/like/:groupId', async (req, res) => {
        ...
        try {
            ...
            if (user.likedGroups.map(id => id.toString()).includes(groupId.toString())) {
            ...
            res.status(500).json({ message: 'Error liking the group', error: error.toString() });
        }
    });
    ...
    userRouter.post('/unlike/:groupId', async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ message: 'Error unliking the group', error: error.toString() });
        }
    });
...
describe('User-Group Like/Unlike Functionality', () => {
    ...
    it('should confirm that the user does not like the group initially', async () => {
        const res = await request(app).get(`/api/user/${user._id}/likes/${group._id}`);
